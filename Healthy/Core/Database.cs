...
namespace Healthy.Core
{
    ...
    public class Database : IDisposable
    {
        ...
        public string GetType(string id, string catId)
        {
            string result = "";
            AddParameter("@id", id);
            AddParameter("@catId", catId);
            DbDataReader r = ExecuteReader("select * from types where id=@id and catId=@catId");
            if (r.Read())
            {

                if (object.ReferenceEquals(r["Title"], DBNull.Value))
                {
                    result = "";
                }
                else
                {
                    result = r["Title"].ToString();
                }
